...
# Конфигурация
PORT = 8080
WORK_DIR = "/var/catty-reminders-app"
APP_DIR = f"{WORK_DIR}/app"
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def _handle_push_event(self, payload):
        ...
        try:
            ...
            if e.returncode == 128:
                print("Репозиторий существует - ок")
                print(e.stdout)
                print(e.stderr)
            else:
                raise
...
def fail_if_work_dir_isnt_writable():
    if not os.access(WORK_DIR, os.W_OK):
        raise RuntimeError(f"{WORK_DIR} is not writable for current user")
...
def main():
    ...
    fail_if_work_dir_isnt_writable()
    try:
